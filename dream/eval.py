...
from cadllm_generate import DreamGenerationMixin
import types
...
class Dream(LM):
    def __init__(
        ...
        # cadllm parameters
        initial_steps: Optional[int] = 128,
        max_steps: Optional[int] = 256,
        initial_block_length: Optional[int] = 48,
        remasking: Optional[str] = 'low_confidence',
        adaptive_blocks: Optional[bool] = True,
        max_block: Optional[int] = 256,
        min_block: Optional[int] = 8,
        confidence_method: Optional[str] = 'softmax',
        adaptive_vocab_size: Optional[bool] = True,
        repetition_detection: Optional[bool] = True,
        adaptive_steps: Optional[bool] = True,
        adaptive_threshold: Optional[bool] = True,
        prophet_enabled: Optional[bool] = False,
        min_confidence_gap: Optional[float] = 0.3,
        prophet_patience: Optional[int] = 2,
        min_confidence_commit: Optional[float] = 0.6,
    ...
    def _generate_batch(self, prompts: List[str]) -> Tuple[List[str], int]:
        ...
        generation_ids = self.model.diffusion_generate(
            ...
            # cadllm parameters
            block_length=self.initial_block_length,
            remasking=self.remasking,
            adaptive_blocks=self.adaptive_blocks,
        ...
        # Report NFE if provided by cadllm
        batch_nfe = 0
        if getattr(generation_ids, "history", None) and isinstance(generation_ids.history, dict) and "nfe" in generation_ids.history:
            batch_nfe = int(generation_ids.history['nfe'])
            print(f"NFE: {batch_nfe}")
        responses = [
    ...
    def generate_until(self, requests: List[Instance], disable_tqdm: bool = False):
        ...
        for batch_idx in range(0, len(requests), self.batch_size):
            ...
            responses, batch_nfe = self._generate_batch(contexts)
            num_nfe += batch_nfe
